from django.conf.urls import url

from . import views

urlpatterns = [
    url(r'^game/(?P<pk>[0-9]+)/$',
        views.GameDetail.as_view(),
        name='game'),
    url(r'^game/$',
        views.GameList.as_view(),
        name='game_list'),
    url(r'^game/search/$', views.search_game, name='game_search'),
    url(r'^player/(?P<pk>[0-9]+)/$',
        views.PlayerDetail.as_view(),
        name='player'),
    url(r'^player/$',
        views.PlayerList.as_view(),
        name='player_list'),
    url(r'^game/search/$', views.search_game, name='player_search'),
    url(r'^event/(?P<pk>[0-9]+)/$',
        views.EventDetail.as_view(),
        name='event'),
    url(r'^event/$',
        views.EventList.as_view(),
        name='event_list'),
    url(r'^game/search/$', views.search_game, name='event_search'),
    url(r'^opening/(?P<pk>[0-9]+)/$',
        views.OpeningDetail.as_view(),
        name='opening'),
    url(r'^opening/$',
        views.OpeningList.as_view(),
        name='opening_list'),
    url(r'^game/search/$', views.search_game, name='opening_search'),
    url(r'^(?P<pk>[0-9]+)/$',
        views.object,
        name='object'),
    url(r'^comment/(?P<pk>[0-9]+)/$',
        views.comment,
        name="comment"),
]
